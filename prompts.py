...
EXTRA_PROMPT_MODE_DESCRIPTIONS: Dict[str, str] = {
    "cinema": (
        f"""
            ...
            Твоя задача — придумать ШЕСТЬ разных коротких текстовых дополнений, которые можно ДОБАВИТЬ В КОНЕЦ основного промта для генерации изображения дома. ВАЖНО изменить окрестности на изображении, исходя из выбранного фильма. ОБЯЗАТЕЛЬНО добавь в окрестности на изображение несколько атрибутов, которые фигурируют в фильме или сериале и ассоциируется с ним. Например, для фильма "Дюна" должны добавиться атрибуты "огромный червь" и "песчаная буря".
